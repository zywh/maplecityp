...
$slng=$_GET["lng"];
	...
	function get_imagelist($county,$ml_num){
		...
		if(is_dir($dir)){
			$picfiles = scandir($dir);
			$num_files = count(scandir($dir)); //there are two DIR returned
		}
		if ( $num_files > 2)    {
			for ($x = 2; $x < $num_files ; $x++) {
				$filePath = $dir.$picfiles[$x];
				array_push($imageList, $filePath);
			}
			
		} else { 
			array_push($imageList,'static/images/zanwu.jpg');
		}
